  ...
  useEffect(() => {
    ...
    const startStream = async () => {
      try {
        ...
        // Retrieve uploads from localStorage to trigger Vision/Docs agents dynamically
        ...
        const response = await fetch(`${BASE_URL}/api/v1/agents/orchestrate/stream`, {
          ...
          body: JSON.stringify({
            ...
            patient_id: analysisId, // Here analysisId is actually patient_id from the URL query params
            ...
            if (part.startsWith('data: ')) {
              try {
                ...
                if (data.status === 'started' || data.status === 'completed') {
                  ...
                  // The backend yields node names like 'supervisor', 'intake', etc.
                  // We map these to our AgentName type.
                  ...
                  if (mappedName) {
                    setAgentStates(prev => {
                      ...
                      // Find the previous active agent and mark it completed if we are moving forward
                      ...
                      if (currentIndex >= 0 && currentIndex < pipeline.length - 1) {
                         ...
                         // Wait, we don't need to push a log for running here, just update the card
                      }

                      // Save to localStorage for RecommendationPage to pick up
