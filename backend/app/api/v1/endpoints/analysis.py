...
@router.post("/start")
def start_analysis(req: AnalysisStartRequest, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def get_analysis(analysis_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    verify_resource_ownership(current_user, analysis.user_id)
    return analysis

@router.get("/history/{patient_id}")
def get_analysis_history(patient_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
