...
async def orchestrate_agents(payload: OrchestrationRequest, current_user=Depends(get_current_user)):
    """
    Triggers the autonomous 11-agent LangGraph orchestration pipeline.
    """
    verify_patient_ownership(current_user, payload.patient_id)
...
async def stream_orchestrate_agents(
    ...
    async def event_generator():
        try:
            ...
            # Run the graph and stream events, keep track of state
            current_state = {
                "session_id": payload.session_id,
                "patient_id": payload.patient_id,
                "raw_prompt": payload.raw_prompt,
            ...
            for event in stream_carepath_agents(
                session_id=payload.session_id,
                patient_id=payload.patient_id,
                raw_prompt=payload.raw_prompt,
                image_urls=payload.uploaded_image_urls,
                ...
                # LangGraph yields a dict with node_name: state_diff
                if isinstance(event, dict):
                    for node_name, state_diff in event.items():
                        # Update current_state with diff
                        current_state.update(state_diff)
                        # We yield a message that this agent has completed its step, plus its reasoning
            ...
            # After the stream finishes, we persist the real result.
            try:
